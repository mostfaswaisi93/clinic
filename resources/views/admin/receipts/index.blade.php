...
@section('title') {{ trans('admin.receipts') }} @endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header border-bottom">
                        <h4 class="card-title"><b>{{ trans('admin.receipts') }}</b></h4>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#data-table').DataTable({
            ...
            columns: [
                ...
                { data: 'action', orderable: false,
                    render: function(data, type, row, meta) {
                        ...
                        return (
                            '<span>' +
                                '@if(auth()->user()->can('update_receipts'))' +
                                    '<a id="'+ row.id +'" name="edit" class="item-edit edit mr-1" data-toggle="modal" data-target="#receiptModal" title="{{ trans("admin.edit") }}">' +
                                    feather.icons['edit'].toSvg({ class: 'font-small-4' }) +
                                    '</a>' +
                                '@endif' +
                            '</span>' +
                            '<span>' +
                                '@if(auth()->user()->can('delete_receipts'))' +
            ...
            buttons: [
                ...
                { text: '<i data-feather="trash-2"></i> {{ trans("admin.trash") }}',
                  className: '@if (auth()->user()->can("trash_receipts")) btn dtbtn btn-sm btn-danger multi_delete @else btn dtbtn btn-sm btn-danger disabled @endif',
                ...
                { text: '<i data-feather="printer"></i> {{ trans("admin.print") }}',
                  className: '@if (auth()->user()->can("print_receipts")) btn dtbtn btn-sm btn-primary @else btn dtbtn btn-sm btn-primary disabled @endif',
                ...
                { text: '<i data-feather="plus"></i> {{ trans("admin.create_receipt") }}',
                  className: '@if (auth()->user()->can("create_receipts")) btn dtbtn btn-sm btn-primary @else btn dtbtn btn-sm btn-primary disabled @endif',
                  attr: {
                    'title': '{{ trans("admin.create_receipt") }}',
                    'data-toggle': 'modal',
        ...
        $(document).on('click', '#create_receipt', function(){
            $('.modal-title').text("{{ trans('admin.create_receipt') }}");
        ...
        $('#receiptForm').on('submit', function(event){
            ...
            if($('#action').val() == "Edit")
            {
                var formData = new FormData(this);
                $.ajax({
                    url: "{{ route('admin.receipts.update') }}",
                    method: "POST",
                    data: formData,
                    contentType: false,
                    cache: false,
        ...
        $(document).on('click', '.edit', function(){
            ...
            $.ajax({
                ...
                success: function(html){
                    ...
                    $('.modal-title').text("{{ trans('admin.edit_receipt') }}");
